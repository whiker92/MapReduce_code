...
public class multilinereg {
	...
	public static void main(String[] args) throws Exception {
		...
		conf.set("thetaPath", "hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start) + "/theta.txt");
		...
		for (int iter = 0; iter < numIter; iter++) {
			...
			if (iter == 0) {
				...
				for (int i = 1; i < numFeature; i++)
					theta += " 0";
				hdfs.mkdirs(new Path("hdfs://localhost:9000/multilineregtheta/"));
				hdfs.mkdirs(new Path("hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start)));
				FSDataOutputStream outputStream = hdfs.create(
						new Path("hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start) + "/theta.txt"));
				outputStream.write(theta.getBytes(), 0, theta.getBytes().length);
				outputStream.close();
			}

			args[1] = "hdfs://localhost:9000/output_multilinereg/" + String.valueOf(start) + "/out";
			...
			if (job.waitForCompletion(true)) {
				System.out.println("Completed iterator " + String.valueOf(iter) + " multi-linear-regression.");
			}
			...
			if (values[0].equals("false") || iter == numIter - 1) { // stop
				...
				hdfs.delete(new Path("hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start)), true);
				hdfs.mkdirs(new Path("hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start)));
				FSDataOutputStream outputStream = hdfs.create(
						new Path("hdfs://localhost:9000/multilineregtheta/" + String.valueOf(start) + "/theta.txt"));
				outputStream.write(ans.getBytes(), 0, ans.getBytes().length);

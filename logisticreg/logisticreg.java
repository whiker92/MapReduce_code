...
public class logisticreg {
	...
	public static class LogisticRegMap extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			for (int i = 0; i < first; i++) {
				Double theta = thetavec.get(i) + Double.parseDouble(alpha) * ((y - h) * x.get(i) + thetavec.get(i));
	...
	public static void main(String[] args) throws Exception {
		...
		args[1] = "hdfs://localhost:9000/output_logisticreg/" + new Date().getTime() + "/out";
		...
		if (job.waitForCompletion(true)) {
			System.out.println("Completed logistic-regression.");
		}
	}
}
